...
void main() {
	...
	case 200: if (Waving_Features) pos += rendLiquid(chunkPos, blockPos.y, Waving_Water, tint); break; // WATER & CAULDRON
	case 191: if (Waving_Features) pos += rendWave(chunkPos, Waving_Objects); break;	// NETHER PORTAL
	case 181: { if (Waving_Features && (absPos.xz == vec2(2) || absPos.xz == vec2(3)) && (absPos.y == 1 || absPos.y == 8 || absPos.y == 10)) {
				pos += rendSwing(blockPos, Waving_Objects);								// LANTERNS
	...
	case 25: // -=- (CUTOUTS) -=-  ----------------------------------------------------------------------------------------
		...
		case 6: { 
					if (Waving_Features) pos += rendWave(chunkPos, Waving_Water * 1.5); 		// SEAGRASS, KELP & LILYPADS
